...
type FuncConfiguration(?requestProcessors, ?responseProcessors) =
  inherit Microsoft.ServiceModel.Http.HostConfiguration()
  ...
  override this.RegisterRequestProcessorsForOperation(operation, processors, mode) =
    requestProcessors' |> Seq.iter (processors.Add << (create operation))

  override this.RegisterResponseProcessorsForOperation(operation, processors, mode) =
